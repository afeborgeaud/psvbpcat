...
c calculate derivatives
	  ...
	  bvecdt(2,m) = ( - x / dsin(theta) * plmdt
     &          + dble(m) * dble(m) / ( 1 - x * x ) * plm(1,m)
     &          - xl2 * plm(1,m) ) * coef * expimp
	  bvecdt(2,-m) = dconjg( bvecdt(2,m) )
	  bvecdt(3,m) = dcmplx( 0.d0, dble(m) ) *
     &     ( - x / ( 1- x * x ) * plm(1,m) 
     &        + 1.d0 / dsin(theta) * plmdt ) * coef * expimp
	  ...
	  bvecdp(3,m) = - dble(m) * dble(m) / dsin(theta) 
     &                  * plm(1,m) * coef * expimp
