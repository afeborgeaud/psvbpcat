...
c            write(*,*) my_rank,ir,indexmap(ir),theta(ir)
     ...
     & bvecdp(3,-2:2,nri))
      allocate(u(9,-2:2,nsta,nri))
      allocate(ulcd(27,-2:2,nsta,nri))
      allocate(outu(nri), outlcd(nri))

c
      memoryperomega = 5*27*16*nsta*nri*0.000001
...
c
      ...
      allocate (outputulcd(27,-1:1,nsta,nri,outputinterval))
      if (iup.eq.0)then
         memoryperomega = (5*(27+9))*16*nsta*nri*0.000001
...
c
      ...
      write(*,*) "allocate outputulcd ",5*27*nsta*nri*outputinterval*
     &  16*0.000001,"Mb"
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
